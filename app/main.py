...
app = FastAPI()
# fs = gcsfs.GCSFileSystem(project='porn-fastapi')
# with fs.open('pornhub-data/pornhub.com-db-shrinked1-0-fixed.csv') as f:
#     dataset = pd.read_csv(f, sep="|")


def recommendation(minDuration, maxDuration, minViews, pornstars, categories) -> pd.DataFrame:
    dataset = pd.read_csv('app/pornhub.com-db-shrinked1-0-fixed.csv', sep="|")
...
def upload(response: Response, req: FilterRequests):
    result = None
    try:
        result = recommendation(req.minDuration, req.maxDuration,\
